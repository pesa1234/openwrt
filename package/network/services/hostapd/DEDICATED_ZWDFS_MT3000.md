...
This is an opt-in MT7981 experiment, restricted to `glinet,gl-mt3000`.
It uses the 5 GHz receiver's third RX path and firmware RDD2 for
background CAC. The MT6000 implementation uses a different MT7986
TX80/RX160 path; see `ADJACENT_ZWDFS_MT6000.md`.

Enable the `mt7915e dedicated_cac=Y` module parameter at boot and the
existing **background radar** checkbox on the 5 GHz radio. The parameter
defaults to `N`. The driver advertises background radar only when the
board matches GL-MT3000 and EEPROM reports at least three 5 GHz RX paths.
The Wi-Fi generator then writes `enable_adjacent_zwdfs=1` and
`enable_background_radar=0` to hostapd. The shared hostapd name refers
to the adjacent DFS *channel block*, not the MT7986 RF implementation.
...
firmware command and RDD2 event routing. This branch adds the board and
EEPROM gate and the third RX path when the module parameter is enabled.

After flashing a debug router, check:

```sh
cat /sys/module/mt7915e/parameters/dedicated_cac
...
Expect `Y`, `enable_adjacent_zwdfs=1`,
...
MT7981 setup. The board and third-RX-path checks here need hardware
